####author : Moahmed Tarek Mohamed < KoToz >
# date : 25/1/2015 >>> project starting point 
...
def get_next_target(page):
	...
	url = [first_quote + 1: end_quote]
	return url , end_quote 

url , end_pos = get_next_target(page)
# here my documentation about tis project 
#get_next_target if a procedure which take page html content 
#and extract it to links starts with "http//" 
#so let's start : 
### line (2) >>> if there is no links inside the page return NONE
### line (5) >>> first_quote >>> get what previously it which http// found 
### line (6) >>> end_quote >>> get ehat previously it which pointed to all link like < https://github.com/ >
### line (7,8,9) >>> this procedure return  two arguments URL and end_quote which point to where's my currently place position . 
# and asignments these results to url , end_pos
#*************************************************
def get_all_links(page):
	...
	while True:
		...
		break
	return links
### line (21) >>> this procedure makaing as a looping to previously procedure get_next_target 
...
def crawl_web(seed):
	...
	while tocrawl:
		...
		if page not in crawled :
			...
			crawled.append(page)
			return index
# we had modifed this procedure from 
#>>> links = [] to  >>> links = {}  ----> which mean that we using dictionary here if you nees to know more visit this site 
[google developers](https://developers.google.com/edu/python/dict-files)			
#***************************************************
def union (p,q):
	for e in q:
		if e not in p:
...
def add_to_index(index , keyword , url):
	if keyword in index :
		...
		index[keyword]=[url]
#****** last one after before using dictionary unit 5 ******
#def add_to_index(index , keyword , url):
...
def get_page(url):
    ...
    return ""
    #******************************************************
    # we are gonna define hash_table which contains all data we need sorting , 
    # so python has a module called (dictionary) which make the followinf procedure but here we use simble ways of listing to find what  # we need 
